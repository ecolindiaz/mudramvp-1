...
const exec = (command, args = [], options = {}) => {
  return new Promise((resolve, reject) => {
    // On Windows, we need to use .cmd for npm and pnpm
    const isWindows = process.platform === 'win32';
    const cmd = isWindows && (command === 'npm' || command === 'pnpm' || command === 'npx') ? `${command}.cmd` : command;
...
async function main() {
  ...
  try {
    ...
    try {
      // Try npm first, then pnpm as fallback
      try {
      ...
      // If both fail, assume dependencies are already installed
    ...
    try {
      let output;
      try {
        output = await exec('npm', ['run', 'setup:autumn']);
      } catch (npmError) {
        output = await exec('pnpm', ['run', 'setup:autumn']);
      }
    ...
    try {
      let output;
      try {
        output = await exec('npm', ['run', 'setup:stripe-portal']);
      } catch (npmError) {
        output = await exec('pnpm', ['run', 'setup:stripe-portal']);
      }
      if (output.includes('[OK]')) {
        console.log('✓ Configured');
      } else {
