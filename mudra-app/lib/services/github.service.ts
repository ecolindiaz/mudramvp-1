...
export async function createOptimizationPR(input: CreateOptimizationPRInput): Promise<PRResult> {
  ...
  // Prepare PR body with improvements summary (code is in the committed files, not duplicated here)
...
> Review the file changes in the **Files changed** tab to see the full implementation.
...
/**
 * Check for existing blog-related files in a GitHub repository.
 * Returns a list of found blog paths so the agent knows what already exists.
 */
export async function checkExistingBlogFiles(brandProfileId: number): Promise<{
  hasBlog: boolean
  foundPaths: string[]
  framework: string
}> {
  const BLOG_CANDIDATE_PATHS = [
    // Next.js App Router
    'app/blog/page.tsx',
    'app/blog/page.jsx',
    'app/blog/page.ts',
  ...
  try {
    const brandProfile = await prisma.brandProfile.findUnique({
      where: { id: brandProfileId },
      include: {
        user: { include: { githubIntegration: true } },
      },
    })

    if (!brandProfile?.user?.githubIntegration) {
      return { hasBlog: false, foundPaths: [], framework: 'unknown' }
    }
    ...
    // Get the repo structure (cached)
    const structure = await getRepoStructure(accessToken, owner, repo, baseBranch)

    // If we have a file list from the repo tree, check against it
    const foundPaths: string[] = []

    if (structure.pageFiles.length > 0) {
      // We got the full tree — do fast local matching
      for (const candidate of BLOG_CANDIDATE_PATHS) {
        if (structure.pageFiles.some(f => f === candidate || f.startsWith(candidate + '/'))) {
          foundPaths.push(candidate)
        }
      }
    } else {
      // Targeted API checks (large repo / truncated tree)
      for (const candidate of BLOG_CANDIDATE_PATHS.slice(0, 10)) {
        try {
          const res = await fetch(
            `https://api.github.com/repos/${owner}/${repo}/contents/${candidate}?ref=${baseBranch}`,
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
                Accept: 'application/vnd.github.v3+json',
          ...
          if (res.ok) foundPaths.push(candidate)
        } catch {
    ...
    return {
      hasBlog: foundPaths.length > 0,
      foundPaths,
      framework: structure.framework,
    }
  } catch (error) {
    console.error('[GitHub] Error checking blog files:', error)
    return { hasBlog: false, foundPaths: [], framework: 'unknown' }
  }
}
